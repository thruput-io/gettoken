...
for fmt in ${PACKAGE_FORMATS:?publish.sh: name the PACKAGE_FORMATS to publish}; do
  case "$fmt" in
    deb)
    ...
    brew)
      mkdir -p "$pages/brew"
      : > "$pages/.nojekyll"
      cp "$root/build/dist/brew/gettoken.rb" "$pages/brew/gettoken.rb"
      echo "brew: formula copied into the site worktree, brew publishing is not built yet"
      ;;
    *)
      echo "publish.sh: no publishing for format '$fmt'" >&2
      exit 1
      ;;
  esac
done
