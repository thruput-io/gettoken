...
# Pattern rules mapping aliases to build artifacts
...
build/bash-unit-test.checked: build/report.tap build/stats.txt thresholds.json
	pass=$$(grep -c -- '^ok ' $<); fail=$$(grep -c -- '^not ok ' $<); \
	min=$$(jq -r '."bash-unit-test".min_tests' thresholds.json); \
	echo "bats: $$pass passed, $$fail failed (min $$min, stat $(BATS_TESTS))"; \
	[ "$$fail" -eq 0 ] && [ "$$pass" -gt "$$min" ] && [ "$$(( $$pass + $$fail ))" -eq "$(BATS_TESTS)" ]

build/bash-coverage.checked: build/kcov/bats/coverage.json build/stats.txt thresholds.json
	percent=$$(jq -r '.percent_covered' $<); \
	files=$$(jq -r '.files | length' $<); \
	floor=$$(jq -r '."bash-coverage".percent' thresholds.json); \
	echo "bash-coverage: $$percent% covered, floor $$floor%, $$files files (stat $(BASH_SOURCE_FILES))"; \
	[ "$${percent%.*}" -ge "$$floor" ] && [ "$$files" -eq "$(BASH_SOURCE_FILES)" ]

build/go-coverage.checked: build/go-coverage.txt build/stats.txt thresholds.json
	percent=$$(grep '^total:' $< | grep -oE '[0-9.]+%$$' | tr -d '%'); \
	files=$$(grep -v '^total:' $< | cut -d: -f1 | sort -u | wc -l); \
	floor=$$(jq -r '."go-coverage".percent' thresholds.json); \
	echo "go-coverage: $$percent% covered, floor $$floor%, $$files files (stat $(GO_SOURCE_FILES))"; \
	[ "$${percent%.*}" -ge "$$floor" ] && [ "$$files" -eq "$(GO_SOURCE_FILES)" ] && [ $$floor -gt 50 ]

build/go-unit-test.checked: build/go-unit-test.json build/stats.txt thresholds.json
	pass=$$(grep -c -- '--- PASS:' $<); fail=$$(grep -c -- '--- FAIL:' $<); ran=$$(grep -c -- '=== RUN' $<); \
	min=$$(jq -r '."go-unit-test".min_tests' thresholds.json); \
	echo "go test: $$pass passed, $$fail failed, $$ran run (min $$min, stat $(GO_TESTS))"; \
	[ "$$fail" -eq 0 ] && [ "$$pass" -gt "$$min" ] && [ "$$ran" -eq "$$(($$pass + $$fail))" ] && [ "$$ran" -eq "$(GO_TESTS)" ]
